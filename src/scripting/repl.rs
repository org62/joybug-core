...
const FN_SIGS: &[FnSig] = &[
    ...
    FnSig { name: "bpi", sig: "\"dll!name\", [handler]", desc: "Breakpoint on an imported API via the debuggee's IAT. No handler = REPL on hit." },
    FnSig { name: "skip", sig: "[ret=0], [args=0], [conv=\"stdcall\"]", desc: "Return from the current call without running it (neutralise an API)." },
    ...
    FnSig { name: "dbg:set_breakpoint_import", sig: "pid, \"dll!name\", [handler(pid,tid,addr)]", desc: "Breakpoint on an import resolved through the debuggee's own IAT (WOW64-safe)." },
    ...
    FnSig { name: "dbg:imports", sig: "pid, [module_base]", desc: "The debuggee's import table with each IAT slot's current target. Default module = main image." },
    FnSig { name: "dbg:skip_call", sig: "pid, tid, [{ret,args,conv}]", desc: "Return from the current call without running it. conv: stdcall/cdecl (x86)." },
    FnSig { name: "dbg:server_info", sig: "", desc: "Server name/version/protocol fingerprint from the handshake." },
    ...
    FnSig { name: "dbg:on_child_ready", sig: "handler(pid, tid, addr)", desc: "Register handler for a CHILD's initial breakpoint (re-arm imports/breakpoints per child)." },
